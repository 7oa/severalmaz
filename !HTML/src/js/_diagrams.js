$(function(){
	...
	for (i=0; i < dials.length; i++){
		...
		var label = $(dials[i]).attr("data-label");
		...
		$(dials[i]).html('<canvas id="dial' + i + '" width="' + size + '" height="' + size + '"></canvas><p>' + procent + '%</p><div class="label">' + label + '</div>');
		...
		context.arc(width/2+lineWidth/2, width/2+lineWidth/2, width/2, 1.5 * Math.PI, 3*Math.PI - (radian+1.5 * Math.PI), true);
		context.lineWidth = 5;
